...
class (Yesod master, PathPiece (AuthId master), RenderMessage master FormMessage) => YesodAuth master where
    ...
    -- >         defaultLoginHandler
    --
    loginHandler :: AuthHandler master TypedContent
    ...
    -- | Called on login error for HTTP HTML requests. By default, calls
    ...
    -- | Called on login error for HTTP JSON requests.
    onErrorJson:: (MonadResourceBase m) => Status -> Text -> HandlerT master m Value
    onErrorJson status msg = sendResponseStatus status $ object [ "message" .= msg ]
...
--
-- Since 1.4.9
defaultLoginHandlerHtml :: AuthHandler master Html
defaultLoginHandlerHtml = do
    tp <- getRouteToParent
    lift . authLayout $ do
...
defaultLoginHandlerJson :: AuthHandler master Value
defaultLoginHandlerJson = do
  tp <- getRouteToParent
  lift . defJson $ do
    master <- getYesod
    mapM_ (flip apLogin tp) (authPlugins master)
  where
    ...
    defJson _ = pure $ object [ "message" .= msg ]

defaultLoginHandler :: AuthHandler master TypedContent
defaultLoginHandler = selectRep $ do
  provideRep defaultLoginHandlerHtml
  provideRep defaultLoginHandlerJson
...
messageJsonStatus :: ( YesodAuth master, MonadResourceBase m )
                  => Status
                  -> Text
                  -> HandlerT master m Html
                  -> HandlerT master m TypedContent
messageJsonStatus status msg html = selectRep $ do
  provideRep html
  provideRep $ onErrorJson status msg
  -- provideRep $ do
    ...
    case auth of
        ...
        UserError msg ->
            case authRoute y of
                ...
                Just ar ->
                  loginErrorMessageMasterI ar msg
        ...
        ServerError msg -> do
            ...
            case authRoute y of
                ...
                Just ar ->
                  loginErrorMessageMasterI ar Msg.AuthError
...
getLoginR :: AuthHandler master TypedContent
